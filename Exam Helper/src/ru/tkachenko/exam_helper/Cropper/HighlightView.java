...
import ru.tkachenko.exam_helper.Activities.MyActivity;
...
class HighlightView {
    ...
    private final Paint oldImage = new Paint();
    private final Paint borderOldImage = new Paint();
    private final Paint textPaint = new Paint();

    public HighlightView(View ctx, Bitmap bitmap) {
        mContext = ctx;
        this.bitmap = bitmap;

        oldImage.setColor(Color.BLACK);
        oldImage.setStrokeWidth(3);
        oldImage.setAlpha(60);
    ...
    public void drawCroppedHighligth() {
        Canvas canvas = new Canvas(bitmap);
        canvas.drawBitmap(bitmap, 0, 0, new Paint());

        canvas.drawRect((int) mCropRect.left, (int) mCropRect.top,
                (int) mCropRect.right, (int) mCropRect.bottom, oldImage);

        canvas.drawRect((int) mCropRect.left, (int) mCropRect.top,
                (int) mCropRect.right, (int) mCropRect.bottom, borderOldImage);

        int l = 10, r = 300;
        String text = Integer.toString(currentTask);
        Rect bounds = new Rect();
        while (r - l > 1) {
            int mid = (r + l) / 2;
            textPaint.setTextSize(mid);
            textPaint.getTextBounds(text, 0, text.length(), bounds);
            if ((bounds.height() < Math.abs(mCropRect.top - mCropRect.bottom)) && (bounds.width() < Math.abs(mCropRect.left - mCropRect.right))) {
                l = mid;
            } else {
                r = mid;
            }
        ...
        textPaint.setTextSize(l / 2);

        canvas.drawText(Integer.toString(currentTask), (int) (mCropRect.left + mCropRect.right) / 2, (int) (mCropRect.top + mCropRect.bottom) / 2, textPaint);
    ...
    void handleMotion(int edge, float dx, float dy) {
        ...
        if (edge == GROW_NONE) {
            ...
            growBy((((edge & GROW_LEFT_EDGE) != 0) ? -1 : 1) * xDelta,
                    (((edge & GROW_TOP_EDGE) != 0) ? -1 : 1) * yDelta, edge);
        }
    }
    ...
    // Grows the cropping rectange by (dx, dy) in image space.
    void growBy(float dx, float dy, int edge) {
    ...
    private void changeEdgeInRect(RectF rect, int edge, double dx, double dy) {
    ...
    public void setup(Matrix m, Rect imageRect, RectF cropRect,
        ...
        mFocusPaint.setARGB(0, 50, 50, 50);
        mOutlinePaint.setStrokeWidth(3F);
        mOutlinePaint.setStyle(Paint.Style.STROKE);
        mOutlinePaint.setAntiAlias(true);

        borderOldImage.setStrokeWidth(3F);
        borderOldImage.setStyle(Paint.Style.STROKE);
        borderOldImage.setAntiAlias(true);
        borderOldImage.setColor(Color.BLACK);

        textPaint.setTypeface(MyActivity.myFont);
